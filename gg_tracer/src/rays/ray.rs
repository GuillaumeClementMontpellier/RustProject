...
use crate::{objects::Element, rays::Dist, utils::constants::SCENE_SIZE};
...
impl<'a> Ray<'a> {
    ...
    pub fn height(&self, fish: bool) -> GameResult<f32> {
        match self.min {
            Some((pt, cible)) => {
                ...
                let longueur_diag =
                    ((SCENE_SIZE.0 * SCENE_SIZE.0 + SCENE_SIZE.1 * SCENE_SIZE.1) as f32).sqrt();
    ...
    pub fn render_3d(&self, ctx: &mut Context, num: f32, width: f32, fish: bool) -> GameResult<()> {
        ...
        if let Err(e) = graphics::rectangle(ctx, DrawMode::Fill, Rect::new(x, y, width, height)) {
            println!("erreur : {}", e);
        }; // unwrap can kill
