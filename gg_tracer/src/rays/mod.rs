use crate::{
    objects::{Element, Intersect},
    utils::constants::{PI, SCENE_SIZE},
};

use ggez::{
    graphics::{self, DrawMode, MeshBuilder, Point2, Vector2},
    Context, GameResult,
};
...
impl Camera {
    ...
    pub fn cast_rays(&mut self, ctx: &mut Context, elem: &[Element], fish: bool) -> GameResult<()> {
        ...
        for (num, angle) in angles.iter().enumerate() {
            ...
            for line in elem.iter() {
                if let Some(point) = line.intersect(&ray) {
                    if let Some((point2, _)) = ray.min {
            ...
            match ray.min {
                ...
                None => {
                    let arr = Point2::from_coordinates(
                        (ray.depart + ray.direction * 10.0 * SCENE_SIZE.0 as f32) - ray.depart,
                    );
            ...
            ray.render_3d(
                ctx,
                num as f32,
                SCENE_SIZE.0 as f32 / (self.nb_rays as f32),
                fish,
            )?;
        }
