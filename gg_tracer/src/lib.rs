use ggez::{
    event::{self, EventHandler, Keycode, MouseState},
    graphics::{self, DrawMode},
    nalgebra::{Rotation2, Translation2},
...
use objects::{line::Line, Element};

pub mod utils;

use utils::constants::{PI, SCENE_SIZE};

pub mod rays;

use rays::{ray::Ray, Camera};
...
impl EventHandler for GameState {
    fn update(&mut self, ctx: &mut Context) -> GameResult<()> {
        ...
        let right = Translation2::from_vector(
            Rotation2::new(PI / 2.0) * self.camera.direction * self.dt * v,
        );
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult<()> {
        ...
        */
        ...
        graphics::circle(
            ...
            ((SCENE_SIZE.0 * SCENE_SIZE.0 + SCENE_SIZE.1 * SCENE_SIZE.1) as f32).sqrt() / 100.0,
            0.1,
        )?;
